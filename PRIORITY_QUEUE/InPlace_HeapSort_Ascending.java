 ...
 */
public class InPlace_HeapSort_Ascending {
    ...
    private static void insert_into_same_heap(int[] a, int i) {
        ...
        while(childindex>0){// same logic as in min priority queue
            if(a[childindex]>a[parentindex]){ //      if(a[childindex]<a[parentindex]){
    ...
    private static int remove_and_add_in_last(int[] a, int heapsize) {
        ...
        while(leftchildindex<heapsize){
            if(a[leftchildindex]>a[maxindex]) // if(a[leftchildindex]<a[maxindex])
                // bas yehi sab change krke ulta hua
                
                maxindex = leftchildindex;
            if(a[rightchildindex]>a[maxindex])
